const apiKey = "33280b3bfe19ebd04438af6e2e020d8e";
...
async function fetchForecast() {
  try {
    ...
    forecasts.forEach((forecast, i) => {
      ...
      cards[i].querySelector(".forecast-temp-min").textContent = Math.round(
        forecast.temp_min
      );
      cards[i].querySelector(".forecast-temp-max").textContent = Math.round(
        forecast.temp_max
      );
    });
  } catch (error) {
...
function updateHumidity(humidity) {
  ...
  document.getElementById(
    "humidity-bar"
  ).className = `${color} h-2.5 rounded-full transition-all duration-300 mt-3`;
...
async function fetchAirQuality(lat, lon) {
  try {
    ...
    document.getElementById(
      "aqi-bar"
    ).className = `${aqiColor}  h-2.5 rounded-full transition-all duration-300 mt-3`;
